...
export default function Testimonials() {
  ...
  const industries = [
    ...new Set(
      displayTestimonials.map((t) => t.industry || t.company).filter(Boolean),
    ),
  ];
  const locations = [
    ...
    <section className="w-full bg-gradient-to-br from-white via-emerald-50/30 to-white py-10 sm:py-14 md:py-20 lg:py-24 overflow-hidden">
      <div className="max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
          ...
          <div className="order-2 grid grid-cols-2 gap-3 xs:gap-4 sm:gap-5 md:gap-6">
            ...
            {industries.length > 0 && (
              <motion.div
                ...
                viewport={{ once: true }}
                className="bg-white rounded-xl sm:rounded-2xl p-3 xs:p-4 sm:p-5 md:p-6 shadow-md sm:shadow-lg border border-gray-100 col-span-2"
              >
                <h4 className="font-semibold text-gray-900 mb-2 sm:mb-3 md:mb-4 text-sm sm:text-base">
                  Trusted By Industries
                </h4>
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {industries.slice(0, 6).map((industry, index) => {
                    // Generate color based on industry name
                    const colors = [
                    ...
                    return (
                      <span
                        key={industry}
                        className={`text-[10px] xs:text-xs px-2 sm:px-3 py-1 sm:py-1.5 rounded-full ${colors[colorIndex]}`}
                      >
                        {industry}
                      </span>
                    );
        ...
        <div className="mt-8 sm:mt-10 md:mt-12 grid grid-cols-2 xs:grid-cols-4 gap-2 xs:gap-3 sm:gap-4 lg:hidden">
          {[
            { value: "CA+CFA", label: "Expert Team" },
